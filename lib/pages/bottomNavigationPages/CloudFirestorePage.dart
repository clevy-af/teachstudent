...
class CloudFirestorePageState extends State<CloudFirestorePage> {
  ...
  monitorAuthenticationState() {
    auth.authStateChanges().listen((user) {
        print("CloudFirestore: User logged ${user != null?'in':'out'}");
      setState(() {
        this.user = user;
      });
    });
  ...
  getData() {
    firestore.collection("election").snapshots().listen((querySnapshot) {
      List<QueryDocumentSnapshot<Map<String,dynamic>>> docSnapshots = querySnapshot.docs;
  ...
  addVote(user, selection) {
    ...
    }).catchError((err) => print("Error voting: " + err)
    );
  }
...
class VoteOption extends StatelessWidget {
  VoteOption({
    ...
    required this.onVote, required this.votes
  }) : super();
